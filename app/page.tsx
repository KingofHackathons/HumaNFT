...
<div className="bg-metaverse bg-bottom bg-no-repeat bg-cover min-h-screen h-5/6">
      <div className="overlay-text h-screen flex flex-col pl-6 bg-slate-800/[0.2]">
        ...
        <h1 className={`${futura.className} text-9xl`}>HumaNFT</h1>
        <p className='text-3xl pl-3'>AI-powered tools to awaken human identities within your NFTs</p>
        <div className='grow'></div>
      </div>
    </div>
  );
 ...
 return (
    <div className={`image-text-section  flex ${imageFirst}`}>
      ...
      <div className="w-1/2 space-y-4">
        <h2 className="text-2xl font-semibold spa">Empower Your NFT Collectibles with Human Identities</h2>
        <p>Your NFT art deserves richer experiences. With HumaNFT, equip each of your NFTs with a unique identity that can converse, interact, and build relationships.</p>
        <p>Add story, lore, and settings to give life to your NFT world. Our AI engine allows your community to collectively author the narratives and dialog so every NFT gains sentience as a human-like character.</p>
        ...
        <p>We can't wait to see the worlds you build when each NFT awakens with its own unique personality. Create human characters that feel alive - join HumaNFT today.</p>
      </div>
    </div>
  );
}
...
function ImageTextSection2({ imageOnLeft = true }) {
  ...
  return (
    <div className={`image-text-section  flex ${imageFirst}`}>
      <div className="image w-1/2">
        <img src='./ThoughtfulApe.png' alt="Thoughtful Ape" />  
      </div>                 <div className="w-1/2 space-y-4 spa">
                    <h2 className="text-2xl font-semibold">Bring Your NFT to Life as a Human Character</h2>
                    <p>Is your NFT art just sitting idle in your wallet? With HumaNFT, you can awaken it with a unique identity for deeper experiences.</p>
                    ...
                    <p>Interact with other NFT characters in a rich collective world. Accomplish challenges, go on quests, gain social followings. Your static NFT will gain sentience as a human persona.</p>
                    ...
                    <p>Help your NFT achieve its destiny as a fully-realized human character. Join HumaNFT and start writing your collective storyverse.</p>
                </div>
    </div>
  );
}
